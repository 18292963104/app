...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("getUserTradeOrder");
  },
  computed: {
    ...mapState({
      addressList: (state) => state.trade.addInfo,
      orderList: (state) => state.trade.tradeOrder,
    }),
    defaultAdd() {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      const { consignee, phoneNum, fullAddress } = this.defaultAdd
